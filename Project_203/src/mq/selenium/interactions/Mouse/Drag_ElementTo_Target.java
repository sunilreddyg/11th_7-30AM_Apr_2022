...
public class Drag_ElementTo_Target {
	...
	public static void main(String[] args) {
		...
		//Create object for mouse and keybaord interactions
		Actions builder=new Actions(driver);
		
		//Drag and drop using direct method
		builder.dragAndDrop(Doc1, TrashBin).perform();
		builder.dragAndDrop(Doc2, TrashBin).perform();
		
		//Drag and drop using mouse interactions method
		builder.clickAndHold(Doc3).moveToElement(TrashBin).release().perform();
		builder.clickAndHold(Doc4).moveToElement(TrashBin).release().build().perform();
	}
